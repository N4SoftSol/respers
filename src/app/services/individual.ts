...
export class IndividualService {
  ...
  whoAmI = httpResource<WhoAmI>(() => {
    ...
    return this.authService.isAuthenticated() ? `${this.baseUrl}/data-app/whoami` : undefined;
  });

  // GET /data-app/individuals
  individuals = httpResource<Individual[]>(() => {
    this.refreshTrigger();
    return this.authService.isAuthenticated() ? `${this.baseUrl}/data-app/individuals` : undefined;
  });
  ...
  create(individual: Individual) {
    return this.http.post<Individual>(`${this.baseUrl}/data-app/individuals`, individual);
  }

  update(id: number, individual: Individual) {
    return this.http.put<Individual>(`${this.baseUrl}/data-app/individuals/${id}`, individual);
  }

  delete(id: number) {
    return this.http.delete<void>(`${this.baseUrl}/data-app/individuals/${id}`);
  }
}
